 ...
 *
 * The db module is mocked so no database is required: we assert on what reaches the model.
 */

const mockUser = {
  create: vi.fn(attrs => Promise.resolve(Object.assign({ id: 1 }, attrs))),
  findByPk: vi.fn(),
  findOne: vi.fn(),
  findOrCreate: vi.fn()
};

// The server code is plain CommonJS loaded through Node's own require, which vi.mock cannot
// intercept — and actually requiring ../db would open a connection and force-sync the test
// database (racing db/models/user.test.js in a parallel worker). Seed the require cache with
// a stub instead, the classic CJS substitute, BEFORE ./auth is required below.
const Module = require('module');
const dbPath = require.resolve('../db');
const dbStub = new Module(dbPath);
dbStub.exports = { model: () => mockUser };
dbStub.loaded = true;
...
beforeAll(() => new Promise(resolve => {
  ...
  // cookie-session + passport middleware here.
