...
describe('security headers (issue #201)', function () {
  it('does not send X-Powered-By', async function () {
    const res = await fetch(baseUrl + '/probe');
    expect(res.status).toBe(200);
    expect(res.headers.get('x-powered-by')).toBeNull();
  });

  it('sets X-Content-Type-Options: nosniff', async function () {
    const res = await fetch(baseUrl + '/probe');
    expect(res.headers.get('x-content-type-options')).toBe('nosniff');
  });

  it('sets a Content-Security-Policy allowing self scripts and websocket connect', async function () {
    const res = await fetch(baseUrl + '/probe');
