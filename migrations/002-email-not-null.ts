...
export async function up ({ context: { queryInterface, Sequelize } }: { context: MigrationContext }): Promise<void> {
  ...
  console.log(`002-email-not-null: deleted ${(meta as { rowCount?: number }).rowCount || 0} row(s) with NULL/empty email`);
...
export async function down ({ context: { queryInterface, Sequelize } }: { context: MigrationContext }): Promise<void> {
  await queryInterface.changeColumn('users', 'email', {
    type: Sequelize.STRING,
    allowNull: true
  });
