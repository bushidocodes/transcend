...
function waitFor (socket: ClientSocket, event: string, ms?: number): Promise<any> {
  ...
  return new Promise(function (resolve, reject) {
    ...
    socket.once(event, function (data: unknown) {
      clearTimeout(timer);
      resolve(data);
    });
  });
...
function handshake (client: ClientSocket, displayName: string, scene?: string, skin?: string): Promise<any> {
...
describe('Socket.io – real-time position sync', function () {
  ...
  function withTwoSubscribers (body: (cA: ConnectedClient, cB: ConnectedClient) => Promise<unknown>) {
    const cA = connect();
    const cB = connect();
