...
export const POSE_FIELDS = ['x', 'y', 'z', 'xrot', 'yrot', 'zrot'];
...
export function pushPose (id, user, t = performance.now()) {
  ...
  for (const field of POSE_FIELDS) {
    snapshot[field] = Number.isFinite(user[field]) ? user[field] : (prev ? prev[field] : 0);
  }
  buf.push(snapshot);
...
export function samplePose (id, now = performance.now()) {
